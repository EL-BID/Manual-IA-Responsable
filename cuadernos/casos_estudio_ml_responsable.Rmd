...
existen agotamientos de inventario:
...
veríamos las siguientes ventas:
...
Sin embargo, si usáramos una política de inventario con 250 unidades, observaríamos:
...
Supongamos que usamos una encuesta por teléfono celular, y que solo
accedimos a zonas que no tienen marginación muy alta.
...
El error en una muestra más similar a la población que se pretende aplicar el algoritmo es mayor:
...
Sobremuestrear clase chica antes de separar datos de validación o hacer validación cruzada puede
producir con estimaciones demasiado optimistas del error de predicción.
...
Supongamos que primero aplicamos (SMOTE)[https://www.jair.org/index.php/jair/article/view/10302]
para intentar balancear los datos:
...
bosque %>%
  ...
  kable()
```
gin with a naive estimate of SATT – the simple difference in means – which would
be useful only if the in-sample distr
Y parece ser que el desempeño es muy bueno. Sabemos que esto es fictico, pues no hay
relación de $y$ con el resto de las covariables.


#### Manera correcta {-}

Antes de hacer el rebalance de clases separamos entrenamiento y validación. Es posible
hacer el muestro estratificado, por ejemplo:
...
no estará disponible al momento de hacer las predicciones (Datos de William Greene's book Econometric Analysis)
