...
datos_ingreso <- encuesta_ingreso %>% 
  ...
  select(ingreso_miles, num_focos, tel_celular, 
         marginacion, ocupadas, piso_firme, automovil, educacion_jef)
```
...
```{r}
library(splines)
formula <- as.formula("log(1 + ingreso_miles) ~ ns(num_focos, 3) + 
              ns(ocupadas, 3) + automovil + piso_firme +
              ns(educacion_jef, 3)")
...
g_sesgo <- ggplot(prueba %>% filter(pred_sesgada < log(30)), 
  aes(x = exp(pred_sesgada) - 1, y = ingreso_miles)) + 
...
g_representativa <- ggplot(prueba %>% filter(pred_sesgada < log(30)), 
  aes(x = exp(pred_rep) - 1, y = ingreso_miles)) + 
  geom_point(alpha = 0.5) +
  geom_abline() + geom_smooth(method = "loess", span = 1) +
  scale_x_log10(limits = c(5, 30)) + scale_y_log10(breaks = breaks_y) +
  xlab("Predicción (miles al trimestre)") +
...
Por ejemplo, para los que tienen ingresos de menos de 20 mil pesos trimestrales,
el modelo sesgado sobrepredice el ingreso verdadero por más del 20%:

```{r}
prueba_bajo <- prueba %>% filter(pred_sesgada < log(20))
...
La razón es que *gasto* se refiere a uso de tarjetas de crédito. Esto incluye
...
datos_roc %>% filter(abs(.threshold - 0.04) < 1e-4) %>% round(4) %>% 
  rename(punto_de_corte = .threshold, espeficidad = specificity, sensibilidad = sensitivity)
```
