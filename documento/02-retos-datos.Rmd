...
## Calidad y relevancia de variable respuesta
...
```{block2, type='rmdtip'}
**Medidas: especificación incorrecta de variable respuesta**
...
```{block2, type='rmdnote'}
**Reto: médición proxy débil respecto a la variable respuesta**
...
otras variables socioeconómicas. Esta pregunta da una medición pobre si nos interesa predecir la probabilidad de
embarazo adolescente según características socioeconómicas. Esta medición puede ser pobre por incentivos de las
...
```{block2, type='rmdtip'}
**Medidas: mala correspondencia de variables respuesta y objetivos**
...
hogares con características particulares (por ejemplo, hogares de personas mayores) pueden tener probabilidad cero de ser seleccionados.
...
para la población objetivo sea muy distinta de la que obtenemos con nuestra muestra natural (ver por ejemplo @anemiaracial, donde se muestra que valores predictivos de anemia pueden ser distintos para distintos grupos raciales). 
...
La situación ideal es la de muestreo probabilístico.  En este caso, podemos entender exactamente qué 
...
- (Cuantitativa) En el caso de intentar inferencia causal con modelos, debe describirse
cuáles fueron las hipótesisi, consideraciones o métodos usados para soportar una interpretación
causal (métodos de matching, balanceo de muestras, etc.)
