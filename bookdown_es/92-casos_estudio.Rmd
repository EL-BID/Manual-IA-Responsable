...
Los cuadernillos utilizan notación con punto decimal para mantener consistencia con los paquetes que así lo usan. Se utiliza el lenguaje de programación R, y los siguientes paquetes:  @tidyverse, 
@recipes, @themis, @rsample, @parsnip, @yardstick, @workflows, @tune, @knitr, 
@patchwork.

Todo el material es reproducible según instrucciones en este [repositorio](https://github.com/EL-BID/Manual-IA-Responsable). El repositorio contiene un archivo Dockerfile que describe las dependencias de infraestructura para su replicación. 
...
## Mala correspondencia de métrica y objetivos {-}
...
Al compararse con el mismo modelo entrenado con una muestra representativa, donde
...
Este ejemplo está tomado de (@ESL y @saheart). Consideramos los siguientes datos donde queremos predecir enfermedad del corazón (chd)^[Datos accesibles en http://archive.ics.uci.edu/ml/datasets/heart+Disease]: 
...
Este ejemplo es originalmente de [@ESL], y se utilizará datos sintéticos, 
generados con el siguiente proceso:

1. Simulando variables respuesta $y$ con distribución binomial,  
2. Simulando 1000 covariables independientes, cada una con distribución normal
estándar.
...
Supóngase que primero aplicamos (SMOTE)[@chawla] para intentar balancear los 
datos:
...
Antes de hacer el rebalanceo de clases se separa en entrenamiento y validación. Si se
...
En este caso se muestra un ejemplo donde se utiliza erróneamente una variable que
...
Lo que indica que algún gasto probablemente incluye el gasto en la tarjeta actual,
...
Supóngase que estamos pensando en un tratamiento para retener estudiantes en algún programa
...
Se puede escoger un punto de corte entre 0.2 y 0.3, por ejemplo, o hacer más simulaciones
para refinar la elección. 

Si se quiere separar el efecto del tratamiento con el efecto del tratamiento aplicado
...
Cuando se tiene un desbalance severo de clases podemos enfrentar dos problemas:
...
Considérense los siguientes datos (del paquete de @ISLR):
...
Pero en prueba los resultados son muy similares. Se agrega también el 
...
Una gráfica de conteos para el score se obtiene un grupo minoritario con valores de la variable score
más baja:
...
y se ve que el grupo naranja también está en desventaja, pues entre los que cumplen hay menos decisiones
de aceptación.
...
Se pueden usar medidas como importancia de permutaciones para examinar modelos. En este ejemplo, se regresa al ejercicio de predicción de aceptación de solicitudes de crédito, y consideramos la importancia basada en permutaciones (@molnar2019):
...
Para explicar predicciones individuales se pueden usar los valores de Shapley (@molnar2019), (@shapley). Estas gráficas indican
...
Considérese el ejemplo de factores para detectar una enfermedad del corazón en el estudio de Sudáfrica que vimos
...
**Observación**: igual que en el modelo y en las gráficas de dependencia parcial que se discutieron anteriormente, 
