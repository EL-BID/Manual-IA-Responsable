...
paquetes que así lo usan. Emplea el lenguaje de programación R y los siguientes paquetes: @tidyverse, 
@recipes, @themis, @rsample, @parsnip, @yardstick, @workflows, @tune, @knitr, 
@patchwork.

Todo el material es reproducible según instrucciones en este [repositorio](https://github.com/EL-BID/Manual-IA-Responsable), el cual contiene un archivo Dockerfile que describe las dependencias de infraestructura para su replicación. 
...
* Para este ejemplo se utilizarán datos de la encuesta nacional de ingresos y gastos en hogares de México [@enigh], para simular el escenario que queremos ejemplificar 
...
2.	Peor aún, el sesgo es mayor para hogares de menores ingresos (las predicciones son altas), lo cual puede producir una focalización mediocre si buscamos identificar hogares de menores ingresos.

### Muestras naturales: comparaciones causales {#natural}{-}

Este ejemplo está tomado de [@ESL] y [@saheart]. Se consideran los siguientes datos, donde el objetivo es predecir enfermedad del corazón (chd)^[Datos accesibles en http://archive.ics.uci.edu/ml/datasets/heart+Disease]: 
...
La interpretación correcta de este gráfica de dependencia parcial [@ESL]
...
**_Selección de variables antes de dividir los datos_** 
...
Este ejemplo es originalmente de [@ESL]. Se utilizarán datos sintéticos, generados con el siguiente proceso: 
...
La selección de variables está dada por la siguiente función. Esta función selecciona las vari - ables más correlacionadas con la variable objetivo: 
...
Supóngase que primero se aplica  (SMOTE)[@chawla] para intentar balancear los 
datos:
...
En primera instancia parece ser que el desempeño es muy bueno. Se sabe que esto 
...
En este caso mostramos un ejemplo donde se utiliza erróneamente una variable que no estará disponible en el momento de hacer las predicciones (datos de [@greene]).
...
Y se obtiene un desempeño pobre según esta matriz de confusión (prueba y entrenamiento). La sensibilidad es muy baja, aunque la especificidad (tasa de correctos negativos) sea alta. Una conclusión típica es que 
*el modelo no tiene valor predictivo*, o que
*es necesario sobre muestrear la clase de ocurrencia baja*.
...
de corte incorrecto**. Un punto de corte de 0.5 con SMOTE equivale a uno mucho más reducido sin
SMOTE.
...
El siguiente ejemplo es derivado de (@hardt). Supóngase que se tiene un atributo protegido A que tiene dos valores: azul y naranja. Naranja es el grupo minoritario desaventajado. Se usan datos simulados como sigue: el atributo score está asociado al atributo protegido. 
...
Se pueden usar medidas como importancia de permutaciones para examinar modelos. En este ejemplo, se regresa al ejercicio de predicción de aceptación de solicitudes de crédito y se considera la importancia basada en permutaciones [@molnar2019]:
